...
bool copyPath(char* original, char* destination);
...
{
    FILE * fp;
    char* newPath = malloc((sizeof(char)*LimitRequestFieldSize)+10);
    strcpy(newPath, path);
...
void list(const char* path)
{
    ...
    char* template = "<html><headF><title>%s</title></head><body><h1>%s</h1><ul>%s</ul></body></html>";
...
const char* lookup(const char* path)
{
    const char* ending = strchr(path, '.');
    if (strncasecmp(".css", ending, 4) == 0)    
        return "text/css";
        
    else if(strncasecmp(".html", ending, 4) == 0)
        return "text/html";

    else if(strncasecmp(".gif", ending, 4) == 0)
        return "image/gif";

    else if(strncasecmp(".ico", ending, 4) == 0)
        return "image/x-icon";

    else if(strncasecmp(".jpg", ending, 4) == 0)
        return "image/jpeg";

    else if(strncasecmp(".js", ending, 4) == 0)
        return "text/javascript";

    else if(strncasecmp(".php", ending, 4) == 0)
        return "text/x-php";

    else if(strncasecmp(".html", ending, 5) == 0)
        return "image/png";
...
{
    //set up for pointer to get and confirm starts with GET
    if (strncmp(line, "GET ", 4) != 0){
        error(405);
        return false;
    }
    
    //setup pointer for request target
    char* reqTarget = strchr(line, ' ');
    
    //copy to abs_path
    bool quotesFound = copyPath(reqTarget, abs_path);
    
    //request target start with '/'
    if (abs_path[0] != '/'){
        error(501);
        return false;
    }
    
    //request target does not have '"'
    if (quotesFound){
        error(400);
        return false;
    }
   
    //copy query out if it exists
    char* queryStart = strchr(reqTarget, '?');
    
    //check query for '"'
    if (queryStart != NULL){
        quotesFound = copyPath(queryStart, query);
    }
    if (quotesFound){
        error(400);
        return false;
